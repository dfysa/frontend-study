...
<div class="news-module">
  <div class="flex align-center" style="margin-bottom: 20px;">
    <img src="../image/sc.png" class="question" />
    <h1 class="module-title">专栏</h1>
  </div>

  <div class="new-topic-container">
    <div class="new-single-topic" v-for="(topic, index) in newsData2.topics" :key="index">
      <el-card class="new-info-card">
        <div class=" align-center">
          <el-avatar class="scavatar" :src="getURL(topic.avatarUrl)" />
          <h2 class="news-name">{{ topic.name }}</h2>
          <p class="news-stats">{{ topic.followers }} 关注 · {{ topic.articles }} 文章</p>
          <p class="news-intro">{{ topic.intro }}</p>
 <div class="button-container">
        <el-button class="new-enter-button" type="text">进入专栏</el-button>
      </div>
        </div>
      </el-card>
    </div>
  </div>
  <el-button type="text" class="bottom-button">查看更多专栏 ></el-button>
</div>
<!-- 专栏结束 -->
  ...
  topics: [
    {
      ...
      followers: "217",
      articles: "1843",
      intro: "缘叶二次元，分享原创动漫评论。同名公众号欢迎关注。",
      avatarUrl: "sc-1.png", // 添加头像 URL
      // 其他数据...
    },
    {
      name: "简七理财",
